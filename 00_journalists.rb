...
def size (journalistes)
	journalistes.size
end

puts "Combien y a-t-il de journalistes dans ce array ? "
puts size (journalistes)



#Combien d'handle contiennent un numéro ?
def liste_avec_num (journalistes)
	journalistes_av_numero = [ ]
	journalistes.each do |journaliste|
		if journaliste =~ /\d/
			journalistes_av_numero << journaliste
		end
	end
		puts journalistes_av_numero
end

#liste_avec_num (journalistes)

def counter (journalistes)
	counter = 0
	journalistes.each do |journaliste|
		if journaliste =~ /\d/
			counter +=1
...
def startmajuscule (journalistes)
	counter = 0
	journalistes.each do |journaliste|
		if journaliste =~ /@[[:upper:]]/
			counter +=1
		end
	end
		puts "#{counter}"
...
startmajuscule (journalistes)

#Combien contiennent une majuscule ?

def majuscule (journalistes)
	counter = 0
	journalistes.each do |journaliste|
		if journaliste =~ /[[:upper:]]/
			counter +=1
...
def alphabétique (journalistes)
	puts journalistes.sort_by {|journaliste| journaliste.downcase}

end
puts"La liste de handle triée par ordre alphabétique"

 alphabétique (journalistes)

 #Quels sont les 50 handles les plus courts de ce array ?
def courts_handles (journalistes)
		journalistes = journalistes.sort_by {|journaliste| journaliste.length}
		journalistes = journalistes [0..50]
		puts"Les plus 50 courts handles sont "
		puts "#{journalistes}"
end

courts_handles (journalistes)
